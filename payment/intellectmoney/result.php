...
function WriteLog($MY_LMI_HASH) {
    ...
    $str = "
	  IntellectMoney Payment Start ------------------
	  date=" . date("F j, Y, g:i a") . "
	  $post
	  MY_LMI_HASH=$MY_LMI_HASH
	  REQUEST_URI=" . $_SERVER['REQUEST_URI'] . "
	  IP=" . $_SERVER['REMOTE_ADDR'] . "
	  IntellectMoney Payment End --------------------
	  ";
    fwrite($handle, $str);
    fclose($handle);
}
...
$LMI_SECRET_KEY = $SysValue['intellectmoney']['LMI_SECRET_KEY'];
...
if (strtoupper($MY_LMI_HASH) == strtoupper((string)$LMI_HASH)) {
    ...
    if ($uid == $new_uid) {
        ...
        if ($num > 0) {
            ...
            Success($uid, $LMI_PAYMENT_AMOUNT, 'intellectmoney');
        }
