...
class PHPShopProductElements extends PHPShopElements {
    ...
    function currency($name='code') {
        ...
        $row=$this->select(array($name), array('id'=>'='.intval($currency)),false,array('limit'=>1),
     ...
     */
     function setCell() {
        ...
        foreach($Arg as $key=>$value)
            if($key<$this->cell and $this->total >= $this->cell) $args[]=$value;
             elseif(!empty($value)) $args[]=$value;
    ...
    function product_grid($dataArray,$cell,$template=false,$line=true) {
        ...
        if(is_array($dataArray)) {
            ...
            foreach($dataArray as $row) {
                ...
                if(empty($template)) $template='main_product_forma_'.$this->cell;
