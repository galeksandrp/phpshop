...
/*
 * unescape[^unescape\(document.cookie|unescape\(pair]
 * iframe[^stwtag("iframe"|XMLHttpRequest]
*/
...
class Guard {
    ...
    var $none_chek_dir=array('UserFiles','pda','install','1cManager','backup','files','csv','editor3');
    var $src=array('php','ini','html','tpl','js');
    var $update_url='http://www.phpshop.ru/update/guard/update.php';
    var $backup_path='./guard/backup/';
    ...
    function start() {
        ...
        if($this->system['enabled'] and empty($this->system['used'])) {
            ...
            if($this->date-$this->system['last_chek']>(86400/$this->system['chek_day_num'])) {
                ...
                if($this->log_id == 1) {
                    $this->create();
                    $this->message('Создание начального бекапа выполнено');
    ...
    // Настройка защитника
    function system() {
        $PHPShopOrm= &new PHPShopOrm($this->SysValue['base']['guard_system']);
            ...
            if(@$db=readDatabase($this->update_url,"virus")) {
                ...
                if(is_array($db)) {
                    ...
                    if($db[0]['status'] != 'passive') {
                        ...
                        $PHPShopOrm->query('TRUNCATE TABLE phpshop_guard_signature');

                        // Вставляем новые сигнатуры
                        foreach($db as $key=>$val) {
                            $PHPShopOrm = &new PHPShopOrm($this->SysValue['base']['guard_signature']);
    ...
    function alert() {
        $PHPShopOrm= &new PHPShopOrm($this->SysValue['base']['guard_log']);
        ...
        if (@$dh = opendir($dir)) {
            while (($file = readdir($dh)) !== false) {
                if ($file != "." && $file != ".." && !in_array($file,$none)) {
                    ...
                    else if(in_array($ext['extension'],$this->src) and file_get_contents($dir."/".$file)) {
    ...
    function chek() {
        $PHPShopOrm= &new PHPShopOrm($this->SysValue['base']['guard_crc']);
    ...
    function create() {
        ...
        // Очищаем
        $PHPShopOrm= &new PHPShopOrm($this->SysValue['base']['guard_crc']);
        $PHPShopOrm->query('TRUNCATE TABLE phpshop_guard_crc');
    ...
    // ZIP
    function zip($dir=false) {
        include($this->classPath."lib/zip/pclzip.lib.php");

        if(empty($dir)) $dir=$this->base;

        $zip_files='';
        $name=date("d-m-Y-H-i",$this->date).'-'.$this->sec;
        if(is_array($dir))
...
бэкап: http://'.$_SERVER['SERVER_NAME'].'/guard/backup/'.$last_backup.'.zip

Если изменения файлов было санкционированным, то перейдите по ссылке для создания нового образа файлов:
http://'.$_SERVER['SERVER_NAME'].'/guard/admin.php?do=create&backup='.$last_backup.'
';
        // Отправить в карантин
        $this->license();
        if(($this->date)<($this->support))
            $content.='
...
http://'.$_SERVER['SERVER_NAME'].'/guard/admin.php?do=quarantine&backup='.$last_backup.'
';
