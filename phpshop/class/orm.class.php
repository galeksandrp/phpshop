...
class PHPShopOrm {
    ...
    function select_native($select=array('*'),$where=false,$order=false,$option=false,$class_name=false,$function_name=false) {
        ...
        if($num > 1000) return $this->_DATA;
        elseif($this->_DATA) return stripslashes_deep($this->_DATA);
...
width="32" height="32" alt="PHPShopOrm Debug On"/ ><strong>'.$name.'</strong>
	 <br><em>'.$action.'</em>';
        if($this->comment) $error.='<em style="color:green"><br>Комментарий: '.$this->comment.'</em></p>';
        else $error.='</p>';
        echo $error;
    }
