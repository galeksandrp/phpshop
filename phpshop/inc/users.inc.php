...
/**
 * Провера авторизации пользователя
 * @package PHPShopCoreDepricated
...
function ChekUsersBase($log,$pas) {
    ...
    if(true_login($log) and true_login($pas)) {
        ...
        if($num>0) {
            $sql="UPDATE ".$SysValue['base']['table_name27']." SET datas='".date("U")."' where id='".$row['id']."'";
            $result=mysql_query($sql);
...
/**
 * Отправление пароля пользователю при восстановлении
 * @package PHPShopCoreDepricated
 * @return string 
 */
function SendUserPassword() {
    ...
    if($_POST['pas_send']==1) {
        ...
        if($num>0) {
            ...
            $header_adm .= "From:   <robot@".str_replace("www.","",$_SERVER['SERVER_NAME']).">\n";
...
к личному кабинету на сайте ".$_SERVER['SERVER_NAME']."
...
/**
 * Проверка пользователя по имени в БД
 * @package PHPShopCoreDepricated
...
/**
 * Запись в БД авторизации ползователя
 * @package PHPShopCoreDepricated
...
where datas<'$nowData' and enabled='0'";
    ...
    if(!empty($name)) {
        ...
        // Обвновляем данные
        $sql="UPDATE ".$SysValue['base']['table_name27']." SET enabled='1', status='".$admoption['user_status']."' where status='$userID'";
        $result=mysql_query($sql);
        $disp='<div id=allspec>
...
<p>Активация пользователя <b>'.$name.'</b> выполнена. Пройдите авторизацию для получения специальных возможностей работы с интернет-магазином.</p>';
    } else {
        $disp='<div id=allspec>
<img src="images/shop/icon_security.gif" alt="" width="16" height="16" border="0" hspace="5" align="absmiddle"><b>Подтверждение регистрации</b>
</div><p>Активация пользователя не выполнена. Пройдите повторную <a href="/users/register.html" class="b">регистрацию</a> для получения специальных возможностей работы с интернет-магазином.</p>';
    }
    return $disp;
}
...
function GetUserRegister() {
    ...
    if($_POST['add_user']==1) $UserAddData=UserAddData();
...
После активации Вам будут доступны дополнительные возможности работы с интернет-магазином.
            ...
            // Заголовки письма менеджеру
            $codepage  = "windows-1251";     
            $header_adm  = "MIME-Version: 1.0\n";
            $header_adm .= "From:  User Activation <robot@".str_replace("www.","",$_SERVER['SERVER_NAME']).">\n";
            $header_adm .= "Content-Type: text/plain; charset=$codepage\n";
            $header_adm .= "X-Mailer: PHP/";
...
".$SysValue['license']['product_name'];
            ...
            $disp='<div id=allspec>
<img src="images/shop/icon_key.gif" alt="" width="16" height="16" border="0" hspace="5" align="absmiddle"><b>Авторизация</b>
</div><p>На ваш адрес <b>'.$_POST['mail_new'].'</b> было отправлено сообщение для активации регистрации пользователя <b>'.$_POST['name_new'].'</b>.<br>
После активации Вам будут доступны дополнительные возможности работы с интернет-магазином.</p>';
            
            // Заголовки покупателю
            $codepage  = "windows-1251";     
            $header_adm  = "MIME-Version: 1.0\n";
            $header_adm .= "From:  User Activation <robot@".str_replace("www.","",$_SERVER['SERVER_NAME']).">\n";
            $header_adm .= "Content-Type: text/plain; charset=$codepage\n";
...
http://".$_SERVER['SERVER_NAME'].$SysValue['dir']['dir']."/users/useractivite.html?key=".md5($_SESSION['sid'])."
После активации Вам будут доступны дополнительные возможности работы с интернет-магазином. 
...
</div><p>Регистрация нового пользователя <b>'.$_POST['name_new'].'</b> прошла успешно.<br>
Пожалуйста авторизуйтесь для доступа к специальным возможностям интернет-магазина.</p>';
    }
    else {
        $disp='<div id=allspec>
<img src="images/shop/icon_key.gif" alt="" width="16" height="16" border="0" hspace="5" align="absmiddle"><b>Авторизация</b>
...
/**
 * Запись изменений персональных данных пользователя
 * @package PHPShopCoreDepricated
...
/**
 * Запись изменений данных авторизации пользователя
 * @package PHPShopCoreDepricated
 * @param int $UsersId ИД пользователя
 * @return string 
 */
...
//mail($LoadItems['System']['adminmail2'],$zag_adm, $content_adm, $header_adm);
    ...
    if ($i) {$display='<H3>История сообщений</H3>
...
/**
 * Вывод личных данных пользователя в кабинете
 * @package PHPShopCoreDepricated
 * @param int $UsersId ИД пользователя
 * @return string 
 */
function UsersRom($UsersId) {
    ...
    // Обновляем данные
    if($_POST['update_user']==1) $UserUpdate=UserUpdateData($UsersId);
    if($_POST['update_password']==1 and $_POST['password_chek']==1) $UserUpdatePassword=UserUpdatePassword($UsersId);
