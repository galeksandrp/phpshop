<?php
/**
 * Навигация по разделу спецпредложений
 * @package PHPShopCoreDepricated
 * @return string
 */
function DispSpecNav(){
    ...
    $num_page=NumFrom("table_name2","where spec='1' and enabled='1'".$sort);
    ...
    while ($i<$num+1) {
        if($i!=$p) {
            ...
            $pageDo=$i*$num_row;
            $navigat.="
	     <a href=\"./spec_".$i.".html".$querystring."\">".$pageOt."-".$pageDo."</a> | ";
        }
        else {
...
<a href=\"./spec_".($p-1).".html".$querystring."\" title=\"Назад\"><img src=\"images/shop/3.gif\" width=\"16\" height=\"15\" border=\"0\" align=\"absmiddle\"></a>
                $navigat<a href=\"./spec_".$p_to.".html".$querystring."\"><img src=\"images/shop/4.gif\" width=\"16\" height=\"15\" border=\"0\" align=\"absmiddle\" title=\"Вперед\"></a>
&nbsp;&nbsp;
<a href=\"./spec_ALL.html".$querystring."\" class=\"$productSortD\">Все позиции</a>
                ";
    }
    return $nava;
}
...
/**
 * Создание SQL запроса раздела спецпредложений
 * @package PHPShopCoreDepricated
 * @return string
 */
function PageSpecDisp(){
    global $SysValue,$LoadItems;
    ...
    // Все страницы
    if($p=="ALL") {
        $sql="select * from ".$SysValue['base']['table_name2']." where spec='1'  and enabled='1' ".$sort." ".$string;
    }
    ...
    elseif(isset($_POST['priceSearch'])) {
        ...
        $sql="select * from ".$SysValue['base']['table_name2']." where  spec='1' and enabled='1' and price BETWEEN ".GetPriceSort($priceOT,0)." AND ".GetPriceSort($priceDO,0)." ".$string;
    }
    else while($q<$p) {
            $sql="select * from ".$SysValue['base']['table_name2']." where spec='1'  and enabled='1' $sort ".$string." LIMIT $num_ot, $num_row";
            $q++;
...
/**
 * Вывод краткого описания товаров раздела спецпредложений
 * @package PHPShopCoreDepricated
 * @return string
 */
function DispSpecKratko(){
    global $SysValue,$LoadItems;
...
<TD width=1  class=setka><IMG height=1 src=\"images/spacer.gif\" width=1></TD>";
    ...
    $SysValue['other']['catalogCategory']="Спецпредложения";
    ...
    $SysValue['other']['productPageNav']=DispSpecNav();
