...
function DispNewNav() {
    ...
    $num_page=NumFrom("table_name2","where newtip='1' and enabled='1'".$sort);
...
/**
 * Создание SQL запроса раздела новинок
 * @package PHPShopCoreDepricated
 * @return string
 */
function PageNewDisp() {
    global $SysValue,$LoadItems;
    ...
    // Все страницы
    if($p=="ALL") {
        $sql="select * from ".$SysValue['base']['table_name2']." where newtip='1'  and enabled='1' ".$sort." ".$string;
    }
    ...
    elseif(isset($_POST['priceSearch'])) {
        ...
        $sql="select * from ".$SysValue['base']['table_name2']." where  newtip='1' and enabled='1' and price BETWEEN ".GetPriceSort($priceOT,0)." AND ".GetPriceSort($priceDO,0)." ".$string;
    }
    else while($q<$p) {
            $sql="select * from ".$SysValue['base']['table_name2']." where newtip='1'  and enabled='1' $sort ".$string." LIMIT $num_ot, $num_row";
            $q++;
...
/**
 * Вывод краткого описания товаров раздела новинок
 * @package PHPShopCoreDepricated
 * @return string
 */
function DispNewKratko() {
    global $SysValue,$LoadItems;
    ...
    while($row = mysql_fetch_array($result)) {
        ...
        if(empty($description))
            $SysValue['other']['productDes']= DispCatSortTable($category,$vendor_array);
...
<TD width=1  class=setka><IMG height=1 src=\"images/spacer.gif\" width=1></TD>";
    ...
    $SysValue['other']['catalogCategory']="Новинки";
