<?php
/**
 * Вывод подробной информации о товаре в печатной форме
...
function DispPrint($n) {
    ...
    if($LoadItems['System']['logo']=="") $SysValue['other']['logoShop']= "images/shop/phpshop_logo.gif";
    ...
    $SysValue['other']['serverShop']=$_SERVER['SERVER_NAME'];
    $SysValue['other']['pathTemplate']=$pathTemplate;

    // Подключаем шаблон
    if(!empty($name)) $disp=ParseTemplate("print/print_page_forma.tpl");
    else $disp=ParseTemplate($SysValue['templates']['error_page_forma']);
