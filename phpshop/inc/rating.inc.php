...
function GetVoteValue($id_charact,$id_good){// Сумма
global $SysValue;
...
function ShowStars($n=0,$id="0",$id_good) {//Преобразование числа в УПРАВЛЯЮЩИЕ звездочки
	for ($y=0; $y<10; $y++) {
		$di=ceil($y/2)*2;
		$yy=$y+1;
		if ($y<$n) {$cc="";} else {$cc="c";} //Включение пустых звездочек
...
function ShowStars2($n=0,$id="0") {//Преобразование числа в звездочки
	for ($y=0; $y<10; $y++) {
		$di=ceil($y/2)*2;
		$yy=$y+1;
		if ($y<$n) {$cc="";} else {$cc="c";} //Включение пустых звездочек
...
function ratingfull() { //Функция вывода всех оценок для товара, с итоговым полем и возможностью переголосования
global $SysValue,$LoadItems;

$disp='';

//Управляющие переменные
$id_good=$SysValue['nav']['id'];
$ids_dir=$LoadItems['Product'][$id_good]['category'];
$id_user=$_SESSION['UsersId'];
$userip=$_SERVER['REMOTE_ADDR'];

if (!$ids_dir) {return "Для данного товара рейтинг не предусмотрен";} //Возврат, если пустая текущая категория, 
//продолжаем если не пустая
...
$result=mysql_query($sql);
@$SysValue['sql']['num']++;
while (@$row = mysql_fetch_array(@$result)) {$id_category.=','.$row['id_category']; $revoting = $row['revoting'];}
$id_category=substr($id_category,1);



if (!$id_category) {return "Для данного товара рейтинг не предусмотрен";} //Возврат, если пустая категория, продолжаем если не пустая
//продолжаем если не пустая
...
if (!$chars_amount) {return "Для данного товара рейтинг не предусмотрен";} //Возврат, если нет характеристик, продолжаем если есть
$i=0;
...
//print_r($avgcount);
	...
	if ($id_user && ((!$yes) || ($yes && $revoting))) { //Управляющие звездочки только зарегистрированным
...
if ($votedate) { //Если уже голосовали
	$done='Вы уже голосовали <BR>'.$votedate.'<BR>';
	if ($revoting) {$done.='Можете переголосовать.';} else {$done.='Переголосование ОТКЛЮЧЕНО';}
...
if (bad==1) {
	...
	for (i=0; i<oknum.length;i++) {
		...
		req.open("POST", "phpshop/ratingvote.php", true);
...
function ratingtop() { //Функция вывода всех оценок для товара, с итоговым полем и возможностью переголосования
global $SysValue;
...
while (@$row = mysql_fetch_array(@$result)) {
	...
	$id=$row['id_good'];
	$name=getgoodname($id);
	if ($name) {@$top.='<TR><TD><A href="shop/UID_'.$id.'.html">'.$name.'</A></TD><TD>'.$rate.'</TD><TD>'.$row['count'].'</TD><TD>'.$row['ttr'].'</TD></TR>';}
}


return '<TABLE border=1>
<TR><TD>Товар</TD><TD>Оценка</TD><TD>Кол-во голосов</TD><TD>Итоговый рейтинг</TD></TR>
'.$top.'</TABLE>';
}//Конец функции


function ratingshort($id_good){//Вывод в кратком выводе
global $SysValue;
...
/////////////////////////////////////////////////////////Функции вывода
//Функция вывода всех оценок для товара, с итоговым полем и возможностью (пере)голосования
//Генерируется в зависимости от текущей навигации, вставляется только в подробном описании
$SysValue['other']['ratingfull']=ratingfull();  

//Функция вывода итогового краткого рейтинга для товара. Должен вызываться из dispkratko с идентификатором товара
//ratingshort(); 

//Функция блок самые высокие оценки товара
$SysValue['other']['ratingtop']=ratingtop();  
