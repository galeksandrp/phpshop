...
//	echo $month.'-'.$n.$sql.'<BR>';
	if ($num_rows) {$row[]='<A href="../news/?timestamp='.$timeststart.'"><B>'.$n.'</B></A>';} else {$row[]= $n;}
...
if (!$year) {$year=date("Y");} //Если не прислали управление годом - активен текущий год
if (!$month) {$month=date("m");}//Если не прислали управление месяцем - активен текущий месяц



$cal = makeCal($year, $month); //Генерация календаря
$timestamp=mktime(0,0,0,$month,1,$year);

foreach ($cal as $row) {
	$disp.='<tr>';
	foreach ($row as $i=>$v) {
		if ($i==6) {$st='style="color:red"';} else {$st="";}
		if (!$v) {$v="&nbsp;";}
		$disp.='<td '.$st.'>'.$v.'</td>';
	}
	$disp.='</tr>';
...
//$monthname=strftime('%B',$timestamp); //Название активного месяца
