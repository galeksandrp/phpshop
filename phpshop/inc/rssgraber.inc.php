...
function add_rss_jurnal($link_id, $status, $last_date, $day_num ){
	...
	if ($status){
		if ($last_date < strtotime(date("Y-m-d"))) $date = strtotime(date("Y-m-d")) + round(86400/$day_num);
		else $date = $last_date + round(86400/$day_num);
	}
	else $date = time();
...
function add_news($news, $num){
	...
	for ($i=$num-1; $i>=0; $i--){
		...
		if ($n == 0) {			
			...
			//$desc1 = utf8_win($news[$i]['description']);
			
			$desc2 = '<a href="'.$news[$i]['link'].'" target="_blank">Источник...</a>';
			
			$sql = "INSERT INTO ".$SysValue['base']['table_name8']." 
			VALUES ('','$date','$title','$desc1','$desc2','$dateU')
			";			
			mysql_query($sql);
...
if(!$row['rss_use']){
	...
	if (count($result))		
		while ($row = mysql_fetch_array($result)){
			...
			$news = parse_rss($row['link'], $row['news_num']);
			if ($news) {
				add_news($news, $row['news_num']);			
				add_rss_jurnal($row['id'], 1, $row['last_load'], $row['day_num'] );
			}
			else add_rss_jurnal($row['id'], 0, $row['last_load'], $row['day_num'] );
		}
...
if($SysValue['nav']['truepath'] == "/" and $admoption['rss_graber_enabled']==1) rss_graber();
?>