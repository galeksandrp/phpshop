...
function actionStart() {
    ...
    $PHPShopInterface->setCaption(array("№ Чека", "15%"),array("Дата", "15%"), array("№ Заказа", "15%"), array("Действие", "10%"),  array("Адрес", "20%"), array("Статус", "10%", array('align' => 'right')));

    $PHPShopOrm = new PHPShopOrm($PHPShopModules->getParam("base.atol.atol_log"));
    ...
    if (is_array($data))
        foreach ($data as $row) {
            ...
            $PHPShopInterface->setRow(array('name' => $row['fiscal'], 'link' => '?path=modules.dir.atol&id=' . $row['id']), PHPShopDate::get($row['date'], true), array('name' => $row['order_uid'], 'link' => '?path=order&id=' . $row['order_id']),$operation, $row['path'], $status);
        }
