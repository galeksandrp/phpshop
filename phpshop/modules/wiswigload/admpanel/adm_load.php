...
function actionStart() {
    ...
    $PHPShopGUI->title="Загрузка визуальных редакторов";
    ...
    $PHPShopGUI->setHeader("Загрузка визуальных редакторов","База доступных редакторов для загрузки",$PHPShopGUI->dir."img/i_display_settings_med[1].gif");

    // Содержание закладки 2
    $Tab2=GetSkins($PHPShopSystem->getSerilizeParam("admoption.editor"));
    $Tab2.=$PHPShopGUI->setField('Скриншот',GetSkinsIcon($PHPShopSystem->getSerilizeParam("admoption.editor")),$float="none",$margin_left=5);

    // Содержание закладки 1
    $Tab1=GetTemplateInfo();
    $Tab1.=$PHPShopGUI->setField('Скриншот',GetSkinsIcon(false,'editor','http://demo.phpshop.ru'),$float="none",$margin_left=5);
    $button=$PHPShopGUI->setInput("submit","editID","Загрузить редактор","right",190,"","but","actionUpdate");
