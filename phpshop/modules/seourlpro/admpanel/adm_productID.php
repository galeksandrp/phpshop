...
function addSeoUrl($data) {
    ...
    if(empty($data['prod_seo_name'])) {
     ...
     $Tab3 = $PHPShopGUI->setField("SEO ссылка:", $PHPShopGUI->setInput("text", "prod_seo_name_new", $data['prod_seo_name'], "left", 400,false,false,false,'http://'.$_SERVER['SERVER_NAME'].'/id/','-'.$data['id'].'.html <p>* Можно использовать вложенные ссылки /sony/plazma/televizor</p>'), "none");
