...
function send_to_order_mod_paypal_hook($obj, $value, $rout) {
    global $PHPShopValutaArray,$papal_delivery;
    ...
    if ($rout == 'START') {
        ...
        $papal_delivery = ' ул. ' . $_POST['adr_name'] . ', д.' . str_replace(array('\\','/'),' к. ',$_POST['PAYMENTREQUEST_0_SHIPTOSTREET2']) . ', кв. ' . $_POST['PAYMENTREQUEST_0_SHIPTOSTREET3'];
        $_POST['adr_name'] = $adr_name . $papal_delivery;
    }

    if ($rout == 'MIDDLE') {


        if ($value['order_metod'] == 10003) {
            ...
            if (empty($option['status'])) {
                ...
                if (strlen($papal_delivery) > 100) {
                    $delivery_1 = substr($papal_deliveryy, 0, 100);
                    $delivery_2 = substr($papal_delivery, 100, 200);
                    ...
                    $requestParams['PAYMENTREQUEST_0_SHIPTOSTREET'] = PHPShopString::win_utf8($papal_delivery);
