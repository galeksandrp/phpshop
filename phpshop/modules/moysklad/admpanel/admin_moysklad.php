...
function actionStart() {
    global $PHPShopInterface, $_classPath;
        ...
        $PHPShopInterface->link = "../modules/moysklad/admpanel/adm_moyskladID.php";
    $PHPShopInterface->setCaption(array("Дата", "10%"), array("Действие", "70%"), array("Статус", "10%"));
    ...
    if (is_array($data))
        foreach ($data as $row) {
            ...
            $PHPShopInterface->setRow($id, PHPShopDate::get($date,true), $order_id, $done);
        }
    ...
    $Calendar = $PHPShopIcon->setForm(
            ...
            $PHPShopIcon->setInput("button", "date_but", "Показать", "right", 70, "DoReload('modules','moysklad','moysklad',calendar.pole1.value,calendar.pole2.value)")
