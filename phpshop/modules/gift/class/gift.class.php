...
class PHPShopGift{
    ...
    function __construct() {
        $PHPShopOrm = new PHPShopOrm($GLOBALS['SysValue']['base']['gift']['gift_forms']);
    ...
    /**
     * Возвращает информацию по действующим подаркам
     */
    function getGift($row) {
            ...
            foreach ($data as $pro) {
                ...
                if ($date_act == 1 && $user_act) {
                    ...
                    if ($sumche == 1 || $sumchep == 1) {
                      return array('id'=>$pro['id'],'gift' => $pro['discount_tip'], 'label' => $pro['label'], 'description' => $pro['description']);
                    }
                }
            }
        }
