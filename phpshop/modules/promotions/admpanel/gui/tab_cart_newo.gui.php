...
function tab_cart_newo($data, $option = false) {
    ...
    // Бланк заказа
    $blank.=$PHPShopGUI->setButton(__('Бланк заказа'), '../img/action_print.gif', 130, 30, $float = "left", $onclick = "DoPrint('/phpshop/modules/promotions/admpanel/formso/forma.php?orderID=" . $data['id'] . "'); return false;");
    // Товарный чек
    $blank.=$PHPShopGUI->setButton(__('Товарный чек'), '../img/action_print.gif', 130, 30, $float = "left", $onclick = "DoPrint('/phpshop/modules/promotions/admpanel/formso/forma2.php?orderID=" . $data['id'] . "'); return false;");
    // Гарантия
    ...
    // Счет
    $blank.=$PHPShopGUI->setButton(__('Счет'), '../img/action_print.gif', 130, 30, $float = "left", $onclick = "DoPrint('/phpshop/modules/promotions/admpanel/formso/account/forma.html?orderId=".$data['id']."&tip=2&datas=".$data['datas']."'); return false;");
    // Счет в сбербанк
    ...
    // Счет-Фактур
    $blank.=$PHPShopGUI->setButton(__('Счет-Фактура'), '../img/action_print.gif', 130, 30, $float = "left", $onclick = "DoPrint('/phpshop/modules/promotions/admpanel/formso/forma4.php?orderID=" . $data['id'] . "'); return false;");
    // Торг-12
    $blank.=$PHPShopGUI->setButton(__('Торг-12'), '../img/action_print.gif', 130, 30, $float = "left", $onclick = "DoPrint('/phpshop/modules/promotions/admpanel/formso/forma5.php?orderID=" . $data['id'] . "'); return false;");
    // Почта
    ...
    if (sizeof($cart) != 0)
        if (is_array($cart))
            foreach ($cart as $val) {
                ...
                $sumn = $val['price'] * $val['num'];
                if($val['discount']!='') {
                    if($val['discount_tip']!='%') {
                        $price_discount = $sumn - ($val['discount']*$val['num']);
                    }
                    else {
                        $di = $val['discount']/100;
                        $price_discount = $sumn - ($sumn*$di);
                    }
                    $sumitog = $price_discount;
                }
                else {
                    $sumitog = $sumn;
                }
                ...
                if($val['discount_tip']!='%') {
                  $val['discount_tip'] = ' '.$PHPShopOrder->default_valuta_code;
                  $val['discount'] = $val['discount']*$val['num'];
                }
...
<tr class=row3 onmouseover=\"show_on('r" . $val['id'] . "')\" id=\"r" . $val['id'] . "\" onmouseout=\"show_out('r" . $val['id'] . "')\" >
 <td style=\"padding:3\">$n</td> 
  ...
  <td style=\"padding:3\">" . $sumitog . "</td>
  <td style=\"padding:3\">" . $val['discount'].$val['discount_tip'] . "</td>
</tr>
";
 ...
 <tr bgcolor=\"ffffff\">
  ...
  <td style=\"padding:3\"></td>
    ...
    if($PERSON['tip_disc']==1) {
      ...
      $tip_disc = ' '.$PHPShopOrder->default_valuta_code;
    }
    $disCart.="
<tr bgcolor=\"#C0D2EC\">
  <td style=\"padding:3\" colspan=\"4\" align=center><span name=txtLang id=txtLang>Итого с учетом общей скидки</span></td>
  <td style=\"padding:3\"><b>" . ($num + 1) . "</b> <span name=txtLang id=txtLang>шт.</span></td>
  <td style=\"padding:3\" colspan=\"3\" align=\"center\"><b>" . ($PHPShopOrder->returnSumma($CART['sum'], $PERSON['discount']) + $GetDeliveryPrice) . "</b> " . $PHPShopOrder->default_valuta_code . "</td>
</tr>
";
...
<tr>
  ...
  <td id=pane align=center><img src=../img/arrow_d.gif width=7 height=7 border=0 hspace=5><span name=txtLang id=txtLang>Cо скидкой</span> " . $PHPShopOrder->default_valuta_code . "</td>
  <td id=pane align=center><img src=../img/arrow_d.gif width=7 height=7 border=0 hspace=5><span name=txtLang id=txtLang>Скидка</span></td>
</tr>
" . $disCart . "
</table>
</div>
