...
function actionStart() {
    ...
    $PHPShopGUI->setHeader("Настройка модуля 'Яндекс.Деньги'", "Настройки подключения", $PHPShopGUI->dir . "img/i_display_settings_med[1].gif");

    $Tab1 = $PHPShopGUI->setField('Наименование типа оплаты', $PHPShopGUI->setInputText(false, 'title_new', $title));
    $Tab1.=$PHPShopGUI->setField('Счет №', $PHPShopGUI->setInputText(false, 'merchant_id_new', $merchant_id, 210), 'left');
    ...
    $Tab3 = $PHPShopGUI->setPay($serial, false, $version, true);
            ...
            $info='Для работы модуля требуется настроить форму HTTP-уведомления по ссылке: <a href="https://sp-money.yandex.ru/myservices/online.xml" target="_blank">https://sp-money.yandex.ru/myservices/online.xml</a>. В качестве адреса для уведомлений нужно указать <b>http://'.$_SERVER['SERVER_NAME'].'/phpshop/modules/yandexmoney/payment/result.php</b>. 
                <p>
