...
class PHPShopNtCatalogElement extends PHPShopShopCatalogElement {
    ...
    /**
     * Вывод навигации каталогов в Топ меню
     * @return string
     */
    function leftCatalNtTop() {
        $this->templateChange = "_top/";
        $dis = $this->leftCatalNt();
        return $dis;
    }
    /**
    ...
    function leftCatalNt($replace = null, $where = null) {
        ...
        if (is_array($this->data)) {
            foreach ($this->data as $row) {
                ...
                if ($this->chek($row['id'])) {
                    $dis.=$this->parseTemplate('catalog/' . $this->templateChange . 'catalog_forma_3_nt.tpl');
                }
                // Если нет подкаталогов
                else {
                    if ($row['vid'] == 1) {
                        $dis.=$this->parseTemplate('catalog/' . $this->templateChange . 'catalog_forma_2_nt.tpl');
                    } else {
                        $dis.=$this->parseTemplate('catalog/' . $this->templateChange . 'catalog_forma_nt.tpl');
                    }
                }
                $i++;
            }
        ...
        if (is_array($data))
            foreach ($data as $row) {
                ...
                if ($flag AND $this->get('catalogPodcatalog3level'))
                    $dis.=ParseTemplateReturn('catalog/' . $this->templateChange . 'podcatalog_forma_nt_2.tpl');
                elseif ($flag)
                    $dis.=ParseTemplateReturn('catalog/' . $this->templateChange . 'podcatalog_forma_nt.tpl');
                else
                    $dis.=ParseTemplateReturn('catalog/' . $this->templateChange . 'podcatalog_forma_3level_nt.tpl');
            }
        return $dis;
    }
...
$PHPShopNtCatalogElement->init('leftCatalNtTop');
