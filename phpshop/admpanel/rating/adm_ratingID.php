...
$sql="select * from ".$SysValue['base']['table_name51']." where id_category='$n' order by num";
$result=mysql_query($sql);
...
// Редактирование записей книги
	  $sql="select * from ".$SysValue['base']['table_name50']." where id_category='$id'";
	$result=mysql_query($sql);
	  $row = mysql_fetch_array($result);
	  ...
	  if($row['revoting']==1)  $sel3="checked"; else  $sel4="checked";
	  ?>
...
while (@$row = mysql_fetch_array($result)) {
	...
	if (strpos($ids_dir,','.$id.',')!==false) {$sel="selected";}else{$sel="";}
