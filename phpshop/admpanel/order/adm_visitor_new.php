...
if(is_numeric(@$orderAdd) and !isset($_POST['productSAVE'])) {
    $visitorUID=CreateNewOrder($orderAdd);
}
elseif(is_numeric(@$userAdd) and !isset($_POST['productSAVE'])) {// Генерим новый на основе данных пользователя
...
// Разбор корзины
        function ViewCart($CART,$PERSON) {
            ...
            @$num=0;
            @$sum=0;
            $GetDeliveryPrice=GetDeliveryPrice($PERSON['dostavka_metod'],0,0);
            $disCart.="
