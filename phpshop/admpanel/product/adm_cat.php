...
function Vivod_rekurs($n) {// вывод подкаталогов рекурсом
    ...
    $sql = "select * from $table_name where parent_to='$n' order by num, name";
    $result = mysql_query($sql);
...
function Vivod_pot() {// вывод каталогов
    global $table_name, $category, $SysValue;
    $sql = "select * from $table_name where parent_to=0 order by num, name";
    $result = mysql_query($sql);
    $i = 0;
    $dis=null;
    while ($row = mysql_fetch_array($result)) {
...
function Num($n) {// ¬ывод кол-ва товаров в катологе
    global $table_name;
    $sql = "select id from $table_name where parent_to='$n'";
    $result = mysql_query($sql);
    $num=0;
...
<html>
    ...
    <body bottommargin="0" leftmargin="5" topmargin="0" rightmargin="5">
        ...
        <table cellpadding="0" cellspacing="5" bgcolor="ffffff" style="border: 1px;border-style: inset;" width="100%" height="350">
            <tr>
                <td valign="top">
<? echo Vivod_pot(); ?>
                </td>
