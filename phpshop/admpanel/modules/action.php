...
if (CheckedRules($UserStatus["module"], 0) == 0)
    switch ($_POST['moduleAction']) {
        case("off"):
            ...
            if (is_array($ModValue['base'][$_POST['moduleId']]))
                foreach ($ModValue['base'][$_POST['moduleId']] as $val)
                    mysql_query("DROP TABLE " . $val);

            // Удаление полей
            if (is_array($ModValue['field'][$_POST['moduleId']]))
                foreach ($ModValue['field'][$_POST['moduleId']] as $key => $val)
                    mysql_query("ALTER TABLE `" . $val . "` DROP `" . $key . "` ");

            // Удаляем дополнительные БД
            $modulePath = "../../modules/" . $_POST['moduleId'] . "/uninstall/module.sql";
