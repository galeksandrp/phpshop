...
function do_encode() {
    ...
    document.getElementById('encoded_text').value = base64_encode(utf8_encode(plain_text));
}

function do_decode() {
    var encoded_text = document.getElementById('encoded_text').value;
    document.getElementById('plain_text').value = utf8_decode(base64_decode(encoded_text));
}

function ReturnBase64(id,val){
alert(id+"-"+val);
}