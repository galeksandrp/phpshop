...
function onCancel() {
    ...
    if (winOpenType == "")
        winOpenType = parent.window.winOpenType;
    switch (winOpenType) {
...
function ReturnPicResize(id) {
    ...
    // есть ли уже главное изображение
    var flag =  document.getElementById('pic_small_new').value.length;
    var path = '../editor3/assetmanager/resize.php?id=' + id +'&flag=' + flag;
...
function EditCatalogPages() {
    ...
    if (window.frame2.document.getElementById("catal") && (window.frame2.document.getElementById("catal").value > 0)) {
        ...
        if (catal != 1000 && catal != 2000)
            miniWin('catalog/adm_catalogID.php?id=' + catal, 650, 630);
    } else
        alert("Выберете подкаталог для редактирования");
...
function NewPhoto() {
    if (window.frame2.document.getElementById("catal") && (window.frame2.document.getElementById("catal").value > 0)) {
        ...
        alert("Выберете подкаталог для добавления фото!");
}
