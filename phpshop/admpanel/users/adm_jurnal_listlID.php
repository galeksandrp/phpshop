...
<html>
    <head>
        <title>Редактирование адреса в черном списке</title>
    ...
    <body bottommargin="0"  topmargin="0" leftmargin="0" rightmargin="0">
        <?
        if (isset($id)) {
            $sql = "select * from " . $SysValue['base']['table_name22'] . " where id=" . intval($_GET['id']);
	   ...
	   ";
                                ...
                                if (@$productDELETE == "doIT") {// Удаление
                                    if (CheckedRules($UserStatus["users"], 1) == 1) {
                                        $sql = "delete from " . $SysValue['base']['table_name22'] . "
where id='$userID'";
                                        $result = mysql_query($sql) or @die("Невозможно изменить запись");
                                        echo"
	  <script>
