...
class PHPShopDone extends PHPShopCore {
    ...
    function send_to_order() {
        ...
        if($this->PHPShopCart->getNum()>0) {
            if(PHPShopSecurity::true_param($_POST['mail'],$_POST['name_person'],$_POST['tel_name'],$_POST['adr_name'],$_POST['tel_name'])) {
    ...
    function write() {
        ...
        // Запись заказа в БД
        $this->PHPShopOrm->insert(array('datas_new'=>time(),'uid_new'=>$this->ouid,'orders_new'=>$this->order,
                'status_new'=>serialize($this->status),'user_new'=>$_SESSION['UsersId']));
    }
