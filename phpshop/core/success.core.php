...
class PHPShopSuccess extends PHPShopCore {
    ...
    function message() {
        ...
        $data=$PHPShopOrm->select(array('*'),array('path'=>'="'.$this->order_metod.'"','enabled'=>"='1'"),false,array('limit'=>1));
        if(is_array($data)) {
    ...
    function write_payment() {
        if($this->out_summ > 0) {
            ...
            $PHPShopOrm->debug=$this->debug;
            $PHPShopOrm->insert(array('inv_id_new'=>$this->inv_id,'name_new'=>$this->order_metod,'sum_new'=>$this->out_summ,'datas_new'=>time()));
        }
    }
    ...
    function get_payment(){
        ...
        $PHPShopOrm = new PHPShopOrm($this->getValue('base.payment_systems'));
        $PHPShopOrm->debug=$this->debug;
        $data=$PHPShopOrm->select(array('path'),array('enabled'=>'="1"','enabled'=>"='1'"),array('order'=>'num'),array('limit'=>100));
        if(is_array($data))
            foreach($data as $val)
