...
class PHPShopPage extends PHPShopCore {
    ...
    function odnotip($row) {
        ...
        if(!empty($odnotipList)) {
            ...
            // Сетка товаров
            $disp=$PHPShopProductIconElements->seamply_forma($data,1,'main_spec_forma_icon');
        }

        if(!empty($disp)) {

            // Вставка в правый столбец
            $this->set('specMainTitle',$this->lang('odnotip'));
            ...
            $this->set('productOdnotip',$this->lang('odnotip'));
    ...
    function ListPage() {
        ...
        // 404
        if(empty($this->category_name)) return $this->setError404();

        // Выборка данных
        $dataArray=$this->PHPShopOrm->select(array('*'),array('category'=>'='.$this->category,'enabled'=>"='1'"),
