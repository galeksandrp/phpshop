...
// Поддержка дорисовки имени главного товара
$parent_add_main_option=false;
...
// Проверка на подтипы
if(true_parent($xid)) $sort=" where uid='$xid'";
else {
    $xid=Chek($xid);
    $sort=" where id=$xid";
}
...
$sql="select * from ".$SysValue['base']['table_name2'].$sort;
...
$same=0;
//если пришла характеристика, подменяем индекс массива
if ($addname) {
    $xid = $xid.'-'.$addname;
}
...
// Дописываем данные в имя
if($parent_add_main_option) {

    $sql="select * from ".$SysValue['base']['table_name2']." where parent REGEXP '".$row['uid']."'";
    $result=mysql_query($sql);
    $row2 = mysql_fetch_array(@$result);
    $cart_new['name']=$row2['name']." ".$cart_new['name'];
    $cart_new['uid']=$row2['uid']." ".$cart_new['name'];
