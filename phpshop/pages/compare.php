...
((($COMCID=="ALL") && (count($_SESSION['compare'])>1) && (count($_SESSION['compare'])<=$limit)))) { //Если выбран каталог сравнения
	...
	if ($COMCID!="ALL") {
		...
		foreach ($cats as $catid => $name) {
			...
			foreach ($tempsorts as $curtempsort) {$sorts[]=$curtempsort;}
		}
	}
    if(is_array($sorts))
	...
	foreach ($sorts_name2 as $name=>$id) {$TDR[0][]=$name;}
	$TDR[0][]='Описание';		
	...
	if ($COMCID!="ALL") {
		...
		foreach ($cats as $catid => $name) {
			foreach ($goods[$catid] as $curtempgood) {$goodstowork[]=$curtempgood;}
		}
	}
...
$price=number_format($price,$format,'.', ' ');
	...
	//Теперь строим таблицу по матрице
	$rows=count($TDR[0]);
	...
	//Конец Теперь строим таблицу по матрице
}

//Если нет товаров, показать пусто. ДОЛЖНО БЫТЬ ПОСЛЕДНЕЙ СТРОКОЙ
if (count($cats)==0) {$disp='<P><h5>Вы не выбрали товары для сравнения!</h5></P>';}
